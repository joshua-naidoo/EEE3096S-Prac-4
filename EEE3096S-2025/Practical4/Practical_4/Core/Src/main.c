...
/* Weak stubs for LCD functions:
   - Provide minimal definitions so project links even if real driver not present.
   - If the real lcd driver provides strong definitions for these symbols,
     those will override these weak versions automatically. */

__attribute__((weak)) void lcd_init(void)
{
  /* Stub: do nothing. If you want to toggle GPIOs here for a visible startup,
     add code. */
}

__attribute__((weak)) void lcd_send_string(const char *s)
{
  /* Stub: suppress unused parameter warning. */
  (void)s;
}
